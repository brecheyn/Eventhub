...
import { Calendar, Users, CheckCircle, TrendingUp, Plus, Clock, MapPin, ArrowRight } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const stats = {
    ...
    publishedEvents: events.filter(e => getActualStatus(e) === 'published').length,
    ongoingEvents: events.filter(e => getActualStatus(e) === 'ongoing').length,
    completedEvents: events.filter(e => getActualStatus(e) === 'completed').length,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="card bg-gradient-to-br from-primary-50 to-white border-2 border-primary-100">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-secondary-900">Participants Total</h3>
                <Users className="h-8 w-8 text-primary-600" />
              </div>
              <p className="text-3xl font-bold text-primary-600">{stats.totalParticipants}</p>
            ...
            <div className="card bg-gradient-to-br from-green-50 to-white border-2 border-green-100">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-secondary-900">Taux de Remplissage</h3>
                <TrendingUp className="h-8 w-8 text-green-600" />
              </div>
              <p className="text-3xl font-bold text-green-600">
                {events.length > 0 
                  ? Math.round((stats.totalParticipants / events.reduce((sum, e) => sum + e.maxCapacity, 0)) * 100)
                  : 0}%
              </p>
              <p className="text-sm text-secondary-600 mt-2">
        ...
        <div className="card">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/events" className="text-primary-600 hover:text-primary-700 font-medium text-sm inline-flex items-center">
              Voir tout
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <span className={`badge ${statusBadge.class} inline-flex items-center`}>
